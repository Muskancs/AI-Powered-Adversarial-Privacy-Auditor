...
print("\nSelect target column for Membership Inference:")
target_col = input("Target column: ").strip()
if target_col not in raw_df.columns:
    raise ValueError("Target column not found.")

print(f"QIs: {qis}, Sensitive: {sensitive_col}, Target: {target_col}")
...
def clean_dataframe(df):
    ...
    for col in df.columns:
        ...
        elif any(k in col.lower() for k in ["email","id","phone","contact"]):
            df[col] = df[col].astype(str).fillna("missing")
        else:
...
def enforce_t_closeness(df, qis, sensitive_col, t=0.2):
    ...
    for key_val, grp in safe_df.groupby("QIKey"):
        loc = grp[sensitive_col].value_counts(normalize=True).reindex(global_dist.index, fill_value=0)
        tvd = 0.5 * (global_dist - loc).abs().sum()
        if tvd > t:
            bad_keys.append(key_val)
    return safe_df[~safe_df["QIKey"].isin(bad_keys)].drop(columns=["QIKey"]).reset_index(drop=True)

# --- Laplace Noise ---
def add_laplace_noise(df, eps=1.0):
    anon = df.copy()
    ...
    for c in numeric_cols:
        ...
        anon[c] = np.clip(col.fillna(np.nanmedian(col)) + noise, vmin, vmax)
...
def k_anonymity_risk(df, qis):
    if df.empty or not qis:
        return {"unique_records": 0, "prop_k<5_records": 0.0, "avg_eq_class_size":0.0, "median_eq_class_size":0.0, "eq_class_sizes":[]}
    key = df[qis].astype(str).agg("|".join, axis=1)
    counts = key.value_counts()
    k1 = int((counts==1).sum())
    prop_small = float(counts[counts<5].sum()/max(len(df),1))
    return {"unique_records": k1, "prop_k<5_records":prop_small, "avg_eq_class_size":float(counts.mean()), "median_eq_class_size":float(counts.median()), "eq_class_sizes":counts.tolist()}
...
def membership_inference_attack(df, target_col):
    ...
    if len(feat_cols)==0: return {"mia_accuracy":0.0,"binary":False}, None, {}
    X = df[feat_cols].fillna(0)
    y_raw = df[target_col].fillna("missing").values
    le = LabelEncoder()
    y = le.fit_transform(y_raw)
    if len(np.unique(y))<2: return {"mia_accuracy":0.0,"binary":False}, None, {}
    class_counts = Counter(y)
    stratify = y if min(class_counts.values())>=2 else None
    X_tr, X_te, y_tr, y_te = train_test_split(X, y, test_size=0.5, random_state=7, stratify=stratify)
    ...
    artifacts = {"X_te":X_te,"y_te":y_te,"preds":preds,"proba":proba,"binary":bool(len(le.classes_)==2)}
...
def attribute_inference_attack(df, sensitive_col):
    ...
    if len(feat_cols)==0: return {"attack_auc":0.5,"attack_acc":0.0,"baseline_acc":0.0}, None, {}
    X = df[feat_cols].fillna(0)
    y_raw = df[sensitive_col].fillna("missing").values
    le = LabelEncoder()
    y = le.fit_transform(y_raw)
    class_counts = Counter(y)
    stratify = y if min(class_counts.values())>=2 else None
    ...
    return {"attack_auc":float(_auc),"attack_acc":float(acc),"baseline_acc":float(baseline)}, pipe, artifacts
...
def overall_risk_score(mia_auc, attr_auc, attr_acc_minus_base, prop_small_eq, weights=(0.35,0.30,0.20,0.15)):
    ...
    w1,w2,w3,w4 = weights
    score = w1*mia_norm + w2*attr_norm + w3*attr_imp + w4*prop_small_eq
    return round(score*100,2)

# Utility & Synthetic Data

def utility_report(raw, other):
    ...
    for c in numeric_cols:
        r = pd.to_numeric(raw[c],errors="coerce").dropna()
        o = pd.to_numeric(other[c],errors="coerce").dropna()
        if len(r)==0 or len(o)==0: continue
        rep["numeric"][c]={"mean_delta":float(abs(r.mean()-o.mean())),
                           "var_delta":float(abs(r.var(ddof=0)-o.var(ddof=0))),
                           "wasserstein":float(wasserstein_distance(r,o))}
    def js_div(p,q):
        m=0.5*(p+q)
        def kl(a,b): a=np.clip(a,1e-12,1); b=np.clip(b,1e-12,1); return np.sum(a*np.log(a/b))
        return 0.5*kl(p,m)+0.5*kl(q,m)
    for c in cat_cols:
        r=raw[c].astype(str); o=other[c].astype(str)
...
def synthesize_tabular(df, n=None, seed=RANDOM_STATE):
    rng=np.random.default_rng(seed); n=n or len(df); syn={}
    for c in df.columns:
        s=df[c]
        if pd.api.types.is_numeric_dtype(s):
            vals=s.dropna().astype(float)
            if len(vals)==0: syn[c]=pd.Series([np.nan]*n); continue
...
def run_audit(df, qis, sensitive_col, target_col):
    ...
    kanon_metrics=k_anonymity_risk(df,qis)
    base_score=overall_risk_score(
        0.5+(mia_metrics["mia_accuracy"]-0.5),
        attr_metrics["attack_auc"],
        attr_metrics["attack_acc"]-attr_metrics["baseline_acc"],
        kanon_metrics["prop_k<5_records"]
    )
    ...
    if anon_df.empty:
        ...
        kanon_metrics_an={"unique_records":0,"prop_k<5_records":0.0,"avg_eq_class_size":0.0,"median_eq_class_size":0.0,"eq_class_sizes":[]}
        anon_score=0.0
        mia_art_an={}
    else:
        ...
        anon_score=overall_risk_score(
            0.5+(mia_metrics_an["mia_accuracy"]-0.5),
            attr_metrics_an["attack_auc"],
            attr_metrics_an["attack_acc"]-attr_metrics_an["baseline_acc"],
            kanon_metrics_an["prop_k<5_records"]
        )
    ...
    if syn_df.empty:
        ...
        syn_kanon={"unique_records":0,"prop_k<5_records":0.0,"avg_eq_class_size":0.0,"median_eq_class_size":0.0,"eq_class_sizes":[]}
    else:
    ...
    return {
        "raw_mia":mia_metrics,"raw_attr":attr_metrics,"raw_kanon":kanon_metrics,"raw_overall_risk_0_100":base_score,
        "anon_mia":mia_metrics_an,"anon_attr":attr_metrics_an,
...
def visualize_privacy(raw_df, results, qis, sensitive_col, save_dir="results/plots"):
    ...
    if sensitive_col in raw_df.columns and not results["anonymized_df"].empty:
        ...
        sns.histplot(raw_df[sensitive_col], kde=True, ax=ax[0], color='skyblue')
        ax[0].set_title("Raw Sensitive Attribute")
        sns.histplot(results["anonymized_df"][sensitive_col], kde=True, ax=ax[1], color='salmon')
        ax[1].set_title("Anonymized Sensitive Attribute")
        plt.tight_layout(); plt.savefig(f"{save_dir}/sensitive_distribution.png"); plt.show()
        ...
        plt.legend(); plt.tight_layout(); plt.savefig(f"{save_dir}/sensitive_distribution_kl.png"); plt.show()

    # Correlation heatmap of QIs
    if qis:
        corr_df = raw_df[qis].apply(lambda x: pd.factorize(x)[0])
        plt.figure(figsize=(8,6))
        sns.heatmap(corr_df.corr(), annot=True, cmap="coolwarm")
        plt.title("Correlation Heatmap of QIs (Raw)")
        plt.tight_layout(); plt.savefig(f"{save_dir}/qi_correlation_heatmap.png"); plt.show()

    # MIA ROC and PR curves
    for suffix, mia_art in [("raw", results.get("mia_artifacts_raw", {})), ("anon", results.get("mia_artifacts_anon", {}))]:
        if mia_art and mia_art.get("binary") and mia_art.get("proba") is not None:
            y_te, y_probs = mia_art["y_te"], mia_art["proba"]
            fpr, tpr, _ = roc_curve(y_te, y_probs)
            roc_auc = auc(fpr, tpr)
            plt.figure(figsize=(6,5))
            plt.plot(fpr, tpr, label=f"AUC={roc_auc:.2f}", color='darkblue' if suffix=="raw" else 'orange')
            plt.plot([0,1],[0,1],'--', color='grey')
            plt.title(f"MIA ROC ({suffix.title()})"); plt.xlabel("FPR"); plt.ylabel("TPR"); plt.legend(); plt.tight_layout()
            plt.savefig(f"{save_dir}/mia_roc_{suffix}.png"); plt.show()

            prec, rec, _ = precision_recall_curve(y_te, y_probs)
            ap = average_precision_score(y_te, y_probs)
            plt.figure(figsize=(6,5))
            plt.plot(rec, prec, label=f"AP={ap:.2f}", color='darkgreen' if suffix=="raw" else 'red')
            plt.title(f"MIA Precision-Recall ({suffix.title()})"); plt.xlabel("Recall"); plt.ylabel("Precision"); plt.legend(); plt.tight_layout()
            plt.savefig(f"{save_dir}/mia_pr_{suffix}.png"); plt.show()
...
def sweep_privacy_utility(df, qis, sensitive_col, target_col, eps_list=(5,2,1,0.5), k_list=(2,3,5)):
    ...
    for k in k_list:
        ...
        for eps in eps_list:
            ...
            if d4.empty:
                records.append({"k": k, "eps": eps, "mia": 0.0, "aia_auc": 0.5, "util_num_wass_med": 1.0})
                continue
            ...
            records.append({"k": k, "eps": eps, "mia": mia["mia_accuracy"], "aia_auc": aia["attack_auc"], "util_num_wass_med": wass_med})
...
def plot_privacy_utility_tradeoff(df_sweep, save_dir="results/plots"):
    ...
    # MIA accuracy vs epsilon
    plt.figure(figsize=(8,6))
    sns.lineplot(data=df_sweep, x="eps", y="mia", hue="k", marker="o", palette="tab10")
    ...
    # Utility (Median Wasserstein) vs epsilon
    plt.figure(figsize=(8,6))
    sns.lineplot(data=df_sweep, x="eps", y="util_num_wass_med", hue="k", marker="o", palette="tab10")
    plt.gca().invert_xaxis()
...
plot_privacy_utility_tradeoff(df_sweep)

# Save anonymized dataset
out_path = "anonymized_dataset.csv"
if not results["anonymized_df"].empty:
    results["anonymized_df"].to_csv(out_path, index=False)
    print(f"Saved anonymized dataset to: {out_path}")
else:
    print("Anonymized dataset is empty; nothing saved.")
